...
The system consists of three major components, which communicated via an API. The first component is the front end, which handles the user interface, and displays the visuals and data sent from the API. It will be written in Javascript, HTML, and CSS and use the React library. The back end communicates with another database (hosted by the same DBMS), which collects survey templates and responses. It will be written in Python, and its endpoints will use the Flask library.
...
Figure 3 abstracts the major functions that are expected to be in the system. This sequence diagram, shown on the next page, illustrates a typical session and is meant to communicate a more detailed view of the components and their relationships. The ``Database'' refers to the back-end database.
...
Sam Elliott helped revise the database schema and made some minor revisions to better meet the rubric. He created a draft of the technology diagram and added many of the images to the document. Sam contributed about 20 percent of the document.
