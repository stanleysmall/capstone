...
Once the requirements and architecture were decided on, we had to design the user interface portion of the product. This is arguably the most important part as this is what the user will be interacting with when using the product. After conferring with our client we decided on a simple UI that would be easy for the user to use and understand. View Appendix ?? to see the full User Interface Design Document.

The Database that will store all our user's class survey data is in development. An API is also in development that will help us interface our system with the existing LimeSurvey system. We have finished a simple prototype that will allow a user to view all screens of the final product. It is not quite entirely interactive at the moment but gives a glimpse of what the final product may become.
