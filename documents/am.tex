...
\begin{enumerate}
    ...
    \item In a terminal program, change your working directory to the folder with the key, and change the key's permissions with the command \verb|chmod 400 limesurvey.pem|. If this command not working in your console, try $path = ".\test.txt" ,icacls.exe $path /reset, icacls.exe $path /GRANT:R "$((Get-Acl-Path .\test.txt).Owner):(R)", icacls.exe $path /inheritance:r . Replace the test.txt with your .pem file, these commands will give you permission to set read-only file.  
...
Yuanqi Guo contributed to the installation section, . Tom contributed about 0 percent of the document.
