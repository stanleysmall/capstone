...
Although there are no known bugs, there are a few limitations in the course evaluation system's functionality, which exist because of time constraints in system development or constraints of the LimeSurvey API. First, only questions that are open-ended or answered on a 1-5 scale are permitted when creating an evaluation form. Second, there is no way to allow for a non-anonymous response that is signed by a student. Third, e-mails that remind students of a published survey are sent to all survey participants instead of just the ones who have not completed the survey. Lastly, the data in both databases is not deleted automatically.

These limitations affect end users in that teacher evaluation forms are less flexible, students may receive unnecessary e-mails, and database data must be deleted manually. Administrators must inform potential users of the evaluation system about the limitations, and users need to take these limitations into account when managing survey forms. As the framework for all of these functions is in place, an administrator would ideally employ a programmer to remove these limitations.
