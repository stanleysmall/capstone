...
\begin{enumerate}
    ...
    \item In a terminal program, change your working directory to the folder with the key, and change the key's permissions with the command \verb|chmod 400 limesurvey.pem|. If the user is using windows 7,8, or 10 and this command does not give you sufficient permissions you can execute \verb|$path = ".\test.txt" ,icacls.exe $path /reset, icacls.exe| \newline \verb|$path /GRANT:R "$((Get-Acl-Path .\test.txt).Owner):(R)", icacls.exe $path /inheritance:r| Replace the test.txt with your .pem file, these commands will give you permission to set read-only files.  
    ...
    \item Route 53 must be configured so \verb|teachingevaluations.org| directs to the AWS instance. One can follow the steps here: \\
...
In order to ensure that the software maintains privacy standards and limit liability for those administering the software the survey data will have to be periodically deleted from the LimeSurvey database.  The administer will need to delete this data by navigating to: \newline http://teachingevaluations.org:5000/index.php/admin/authentication/sa/login and following the on screen instructions to delete the stored data.  In the future this tasks will be automated to reduce the need for administrator interaction as much as possible. Both the system database and the Limesurvey database should be backed up periodically. Refer to the installation instructions in the installation guide for database access. 

\subsection{User Support}

Administrators can turn to available resources for help with managing the course evaluation system. The GitHub repository of the system, at https://github.com/stanleysmall/capstone, contains a README that has installation instructions, as well as commands to build the code or mount the system directory on a local machine. The source code contains robust comments on purpose and functionality of code, how to format endpoint calls and what the expected JSON input is.  If the documentation doesn't provide sufficient support administrators can open issues on github for the developers to address by clicking on the issues tab and then the new issue button. For personal help, one can contact the team leader, Stanley Small, through e-mail at stanley.small@maine.edu.

\section{Troubleshooting}

\subsection{Dealing with Error Messages and Failures}

Administrators will need to handle errors that occur during the system's operation. After accessing the web server with the SSH command the terminal window will show error information that describes what went wrong. This is prefixed by the name of a Docker container, which lets one know which component of the system has the error. Upon an error, an administrator must first check whether he or she has followed the installation procedure correctly.

If the installation procedure is followed correctly the user should first attempt restarting the system.  After accessing the web server via SSH the administrator should execute \verb|docker-compose stop| to stop the current running process.  The Administrator can then start the instance again by entering \verb|docker-compose up -d|.  If the problem persists it most likely lies in the system's code. Administrators can open an issue on the projects github or refer the error information printed by the console to an programmer who is able to alter the source code of the application. Error messages state which line in a file is causing a problem, the source code comments can help with identifying what the issues. Stack Overflow is also a helpful website for diagnosing error messages output by a computer program. As a last resort, administrators should message Stanley Small with the error message.
...
This page shows that all members of Team EVAL and the client, Harlan Onsrud, have agreed on all the information in the administrator manual. By signing this document, Team EVAL and Dr. Onsrud approve the information given to system administrators about how to manage the course evaluation system in right way.
The team will follow a process in the case that the document is changed after we sign it. First, the team will write a rough draft of the changes to be made to the document. Second, all team members and Harlan Onsrud will sign the document agreeing to the changes. Finally, the team will make the changes to the final copy of the document. The estimated date for final report will be 4/25, after that team members stop coding for Course Evaluation System.
...
Stanley Small contributed to the routine tasks section, the periodic administration section, the purpose of this document, and the background section, and he revised the installation section. Stan contributed about 25 percent of the document.
...
Yuanqi Guo added to the installation section. Tom contributed about 2.5 percent of the document.
...
Robert Judkins proofread the document and made changes to improve readability. Robert contributed about 2.5 percent of the document.
